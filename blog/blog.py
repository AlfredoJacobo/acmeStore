...
class Edit:
    def GET(self, id):
        id_producto = int(id)
        productos = model.get_producto(id_producto)
        form = New.form()
        form.fill(post)
        return render.edit(productos, form)
    ...
    def POST(self, id):
        ...
        model.update_producto(id_producto(),
            form.d.producto,
            form.d.descripcion,
            form.d.existencias,
            form.d.precio_compra,
            form.d.precio_venta,
            form.d.imagen_producto
            )
        raise web.seeother('/')
...
class Delete:
    def POST(self, id):
        id_producto = int(id)
        productos = model.get_producto(id_producto)
        model.delete_producto(id_producto)
        raise web.seeother('/')
